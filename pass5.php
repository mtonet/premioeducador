...
if ($action == 'save') {
	if ( isset($_FILES['doc']) ) {
		...
		$fileName = $facadeDadosTrabalho->trataTxt($fileName);
...
?>
        ...
        <div class="legenda" style="font-size: 125%<?php if ($file_type_error): ?>; color: red<?php endif; ?>">Ser&aacute; aceito apenas um arquivo em Word ou PDF (extens&otilde;es .doc, .docx, ou .pdf) com tamanho m&aacute;ximo de 100 KB. N&atilde;o devem ser inseridos no arquivo: fotos, links de v&iacute;deos, etc.</div>
		  
		
		<?php if ($facadeDadosTrabalho->verificaJaCadastrado($id) > 0): ?>
        <div class="legenda" style="font-size: 125%">Importante: Revise o arquivo antes de realizar o envio.<br />
         
        <a href="upload/<?php echo $trabalho_line['nome_arquivo']; ?>" class="btn" target="_blank">Visualizar trabalho enviado</a></div>
		<?php endif; ?>
		...
		<?php if ($big_file_size_error): ?>
		<div class="legenda" style="color: red">Arquivo com mais de 100 kB.</div><?php endif; ?>
        ...
        <form class="form-horizontal" action="pass5.php" method="POST" id="pass5-form" enctype="multipart/form-data">
          ...
          <fieldset>
			...
			<br /><button type="submit" class="btn" style=" margin:5px 0 0 0;">
            
            <?php if ($facadeDadosTrabalho->verificaJaCadastrado($id) > 0) { ?>
            Subistituir Trabalho Anexado
            <?php  } else {  ?>
			Enviar trabalho
			<?php }  ?>
            ...
            <?php if ($_SESSION['ultimo_passo'] == 6): ?><button class="btn btn-primary" id="pass6-button">Avan&ccedil;ar</button><?php endif; ?>
          	</div>
