...
header("Content-Disposition: attachment; filename=convidados-presentes-premiovictor-" . date("Y-m-d-h-i-s") . $ext);
...
$presentes      = isset($_REQUEST['presentes']) ? $_REQUEST['presentes'] : 'sim';
...
if($busca!=''){
	...
	if ($presentes != "")
		$get["presentes"] = $presentes;

	//get list of controleDeAcesso
	$result = $facadeControleDeAcesso->getPresentList($ordem, $nome, $codigo, $acompanhante, $confirmados, $presentes, '', $paginaAtual, $paginaLimite);
	$total = $facadeControleDeAcesso->getTotalList($nome, $codigo, $acompanhante, $confirmados, $presentes);
}
