...
require_once(dirname(__FILE__).'/../../dao/daoUsuario.class.php');
require_once(dirname(__FILE__).'/../../dao/daoInscrito.class.php');
require_once(dirname(__FILE__).'/../../dao/daoDadosGestor.class.php');
require_once(dirname(__FILE__).'/../../dao/daoDadosProfessor.class.php');
require_once(dirname(__FILE__).'/../../dao/daoDadosAcademicos.class.php');
require_once(dirname(__FILE__).'/../../dao/daoDadosProfissionais.class.php');
require_once(dirname(__FILE__).'/../../dao/daoDadosCadastrais.class.php');
require_once(dirname(__FILE__).'/../../dao/daoDadosTrabalho.class.php');
...
function validaUsuarioAdmin($pagina)
{
	...
	if ( !isset($_SESSION['usuario_id']) ) {
		header("Location: ".SITE_URL."/controledeacesso/admin");
	}
	
	if( ($pagina == "inserir-convidados" || $pagina == "editar-convidados" || $pagina == "exec-inserir-convidados" || $pagina == "exec-editar-convidados") && $_SESSION['edicao_inscritos'] == 0)
		header("Location: ".SITE_URL."/controledeacesso/admin");
	elseif( ($pagina == "dados-cadastrais" || $pagina == "convidado-inserido") && $_SESSION['lista_inscritos'] == 0)
		header("Location: ".SITE_URL."/controledeacesso/admin");
	elseif( ($pagina == "relatorios-controle-de-acesso" || $pagina == "convidados-export") && $_SESSION['relatorios'] == 0)
		header("Location: ".SITE_URL."/controledeacesso/admin");
}
...
function getTotalPaginas($rs, $registros_por_pagina){
	$total_de_registros = is_numeric($rs) ? $rs : mysql_num_rows($rs);
...
/*
 * Gera código para novo convidado
 * */

if(isset($_GET['cdg']) && $_GET['cdg'] == "gr"){
    gera_codigo($_GET['origem'], $_GET['vip']);
}

function gera_codigo($origem, $vip) {

    $codigo = rand(0,9).rand(0,9).rand(0,9).rand(0,9);

    $result = encontra_codigo($origem.$codigo.$vip);

    if($result)
        gera_codigo($origem, $vip);

    die($origem.$codigo.$vip);
}

function encontra_codigo($codigo)
{
    $banco = new DAO('../dao/config.xml');

    $banco->conecta();

    $result = $banco->executaQuery("SELECT id FROM controle_de_acesso WHERE codigo = '".$codigo."'");
...
function retorna_origens()
{
    $banco = new DAO('../dao/config.xml');

    $banco->conecta();

    $result = $banco->executaQuery("SELECT id, descricao FROM origem WHERE ativo = '1'");
    while($retorno[] = mysql_fetch_object($result));
