...
$facadeInscrito->lembretecadastrofinalizado($_REQUEST['id']);

echo("<script>alert('Lembrete enviado com sucesso');</script>");

echo("<script>window.location = 'lista-inscritos.php';</script>");
...
$result = $facadeInscrito->getListNoDisablednovo($estado, $esc_categoria, $prof_disciplina, $categoria, $segmento, $atuacao, $status, $texto, $paginaAtual, $paginaLimite);
...
?>
			...
			<header class="topo-float">
            	<h1  style="width:450px;">Lista de inscritos</h1>
                <div class="subnav">
            ...
            <button type="submit" class="btn">Buscar</button>
            	...
            	<p style="font-size:14px;">Filtrar por:</p>
                    ...
                    <select id="segmento" name="segmento">
                        ...
                        <option value="EIC" <?php if ($segmento === 'EI') echo "selected"; ?>>Educação Infantil: Creche</option>
                         <option value="EIP" <?php if ($segmento === 'EI') echo "selected"; ?>>Educação Infantil: Pré-escola</option>                       
              ...
              <a href="ver-nao-finalizados.php" style="margin: 0 0 0 30px">Lista de não-finalizados</a>
					  ...
					  <?php 
					  $res = $facadeInscrito->getListNoDisablednovo($estado, $esc_categoria, $prof_disciplina, $categoria, $segmento, $atuacao, $status, $texto)
				...
				//	$resultstatus = $facadeInscrito->verificaStatus($line['id']);
					...
					?>
                      <tr>
                        ...
                        <td><a href="ver-status.php?id=<?php echo $line['id']; ?>"><span id="printstatus<?php echo($countline); ?>"><?php print_status_novo($facadeInscrito->verificaStatus($line['id'])); ?></span></a></td>
                        <td><?php echo utf8_encode($line['nome']); ?></td>
                        <td><?php echo utf8_encode($line['email']); ?></td>
                        <td><?php echo utf8_encode($line['senha']); ?></td>
						...
						<?php if ($line['ultimo_passo'] >= 5): ?><a target="_blank"  href="ficha-inscricao.php?id=<?php echo $line['id']; ?>" onClick="reloadpg(<?php echo($countline); ?>)"><i id="iconeprint<?php echo($countline); ?>" class="icon-print<?php if ($line['status'] > 0) echo " done"; ?>"></i></a><?php endif; ?>
                        <script>
						function reloadpg(obj){
						 document.getElementById('iconeprint'+obj).className = 'icon-print done';
						 document.getElementById('printstatus'+obj).innerHTML = 'Ficha-impressa';
						 //alert('entrou'+obj);
						}
						</script>
                        </td>
...
<?php if ($line['ultimo_passo'] >= 5): ?>
        <a target="_blank" class="trabalho-download" data-id="<?php echo $line['id']; ?>" data-url="../upload/<?php echo htmlentities(urlencode($line['nome_arquivo'])); ?>"  href="downloadprojeto.php?id=<?php echo $line['id']; ?>&arquivo=../upload/<?php echo htmlentities(urlencode($line['nome_arquivo'])); ?>" onClick="downloadpg(<?php echo($countline); ?>)">
                <i id="icondownload<?php echo($countline); ?>"  class="icon-download<?php if ($line['status'] == 2) echo " done"; ?>"></i>
        </a>
<?php endif; ?>
                        <script>
						function downloadpg(obj){
						 document.getElementById('icondownload'+obj).className = 'icon-download done';
						 document.getElementById('printstatus'+obj).innerHTML = 'Trabalho-impresso';
						 //alert('entrou'+obj);
						}
						</script>                        
                        ...
                        <td><a target="_self" href="lista-inscritos.php?id=<?php echo $line['id']; ?>&action=enviar"><i class="icon-download"></i></a></td>                        
                   ...
                   <div class="pagination" align="center" style="margin-top:30px;">
                    <ul>
				 <?php echo htmlPaginacao($_GET['paginaAtual'], $paginaLimite, $facadeInscrito->getListNoDisablednovo($estado, $esc_categoria, $prof_disciplina, $categoria, $segmento, $atuacao, $status, $texto, 0,0), $get) ?>
				  </ul>
                  </div>
