...
$facadeInscrito->lembretecadastro($_REQUEST['id']);

echo("<script>alert('Lembrete enviado com sucesso');</script>");

echo("<script>window.location = 'ver-nao-finalizados.php';</script>");
...
$result = $facadeInscrito->getListNaoFinalizados($estado, $esc_categoria, $prof_disciplina, $categoria, $segmento, $atuacao, $status, $texto, $paginaAtual, $paginaLimite);

include("header.php");
?>

			<header class="topo-float">
            	<h1>Relatório de inscrições não-finalizadas</h1>
                <div class="subnav">
			...
			<form action="ver-nao-finalizados.php?paginaAtual=" method="POST" id="form">
            ...
            <button type="submit" class="btn">Buscar</button>
            	...
            	<p style="font-size:14px;">Filtrar por:</p>
                    ...
                    <select id="select01" style="margin:0 29px 0 0;" name="status" disabled>
                        <option value="" selected>Não-Finalizados</option>
                      </select>
              ...
              <a href="ver-desativados.php" style="margin: 0 0 0 555px">Lista de desativados</a>
					  ...
					  <?php 
					  $res = $facadeInscrito->getListNaoFinalizados($estado, $esc_categoria, $prof_disciplina, $categoria, $segmento, $atuacao, $status, $texto)
					...
					<?php while ( $line = mysql_fetch_array($result) ): ?>
                      <tr>
                        ...
                        <td><?php if ($line['ultimo_passo'] >= 5): ?><a target="_blank" href="ficha-inscricao.php?id=<?php echo $line['id']; ?>"><i class="icon-print<?php if ($line['status'] > 0) echo " done"; ?>"></i></a><?php endif; ?></td>
                        <td><?php echo $line['ultimo_passo']; ?></td>
                        <td><a target="_self" href="ver-nao-finalizados.php?id=<?php echo $line['id']; ?>&action=enviar"><i class="icon-download"></i></a></td>
                        <td><?php if ($line['ultimo_passo'] > 5): ?><a class="trabalho-download" data-id="<?php echo $line['id']; ?>" data-url="../upload/<?php echo htmlentities(urlencode($line['nome_arquivo'])); ?>"><i class="icon-download<?php if ($line['trabalho_status'] == 1) echo " done"; ?>"></i></a><?php endif; ?></td>
                      </tr>
					<?php endwhile; ?>
                    </tbody>
      				</table>
            <form id="planilha" action="gerar-planilha.php" method="POST">
              <input type="hidden" name="nao_finalizados" value="1" />
           ...
           <div class="pagination" align="center" style="margin-top:30px;">
                    <ul>
				 <?php echo htmlPaginacao($_GET['paginaAtual'], $paginaLimite, $facadeInscrito->getListNaoFinalizados($estado, $esc_categoria, $prof_disciplina, $categoria, $segmento, $atuacao, $status, $texto, 0,0), $get) ?>
				  </ul>
                  </div>
