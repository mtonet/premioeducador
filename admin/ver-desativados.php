...
$result = $facadeInscrito->getList($estado, $esc_categoria, $prof_disciplina, $categoria, $segmento, $atuacao, $status, $texto, $paginaAtual, $paginaLimite);

include("header.php");
?>

			<header class="topo-float">
            	<h1>Lista de inscritos desativados</h1>
                <div class="subnav">
			...
			<form action="ver-desativados.php?paginaAtual=" method="POST" id="form">
            ...
            <button type="submit" class="btn">Buscar</button>
              ...
              <a href="ver-nao-finalizados.php" style="margin: 0 0 0 555px">Lista de não-finalizados</a>
					  </p>
					  </form>
					  ...
					  <?php 
					  $res = $facadeInscrito->getList($estado, $esc_categoria, $prof_disciplina, $categoria, $segmento, $atuacao, $status, $texto)
					...
					<?php while ( $line = mysql_fetch_array($result) ): ?>
                      <tr>
                        ...
                        <td><?php if ($line['ultimo_passo'] >= 5): ?><a target="_blank" class="trabalho-download" data-id="<?php echo $line['id']; ?>" data-url="ficha-inscricao.php?id=<?php echo $line['id']; ?>"><i class="icon-print<?php if ($line['status'] > 0) echo " done"; ?>"></i></a><?php endif; ?></td>
                        <td><?php if ($line['ultimo_passo'] > 5): ?><a href="../upload/<?php echo htmlentities(urlencode($line['nome_arquivo'])); ?>"><i class="icon-download<?php if ($line['trabalho_status'] == 1) echo " done"; ?>"></i></a><?php endif; ?></td>
                   ...
                   <div class="pagination" align="center" style="margin-top:30px;">
                    <ul>
				 <?php echo htmlPaginacao($_GET['paginaAtual'], $paginaLimite, $facadeInscrito->getList($estado, $esc_categoria, $prof_disciplina, $categoria, $segmento, $atuacao, $status, $texto, 0,0), $get) ?>
				  </ul>
                  </div>
