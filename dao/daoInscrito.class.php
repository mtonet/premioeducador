...
require_once $_SERVER['DOCUMENT_ROOT'].'/business/facadestatus.php';
...
function enviar_lembrete_certificado($nome, $id, $email) {
	...
	$mail->AddAddress('marcelotonet@gmail.com', $nome);
	...
	$mail->Subject = "Certificado de Participação 18º Prêmio Educador Nota 10"; // Assunto da mensagem
	$mail->Body = $conteudo;
	//$mail->AltBody = "Este é o corpo da mensagem de teste, em Texto Plano! \r\n <img src="http://blog.thiagobelem.net/wp-includes/images/smilies/icon_smile.gif" alt=":)" class="wp-smiley"> ";
...
class InscritoDAO {
	...
	public function getInscritoStatus($id) {
		try {
			$retorno = 0;
			$this->banco->conecta();
 
			$result = $this->banco->executaQuery("select status from inscricao_status where inscrito_id = '".$id."' order by id desc limit 1");
...
public function lembretecertificadonv($id) {
		try {
			$retorno = 0;
			$this->banco->conecta();
		...
		//	$this->banco->executaQuerySemRetorno("UPDATE inscrito SET senha='" . mt_rand(1000, 99999999) . "' WHERE cpf='$cpf'");
			...
			enviar_lembrete_certificado($nome, $id, "marcelotonet@gmail.com");
...
case 'EIP':
	...
	public function getListInscritosNoDisablednovo($estado, $esc_categoria, $prof_disciplina, $categoria, $segmento, $atuacao, $status, $texto='', $paginaAtual=0, $paginaLimite=0) {
		try {
			 $this->banco->conecta();
			 
			 $inicioPagina = ($paginaAtual*$paginaLimite)-$paginaLimite;
			 $fimPagina = ($inicioPagina+1) + $paginaLimite;
			 $viewbanco = "vw_inscrito_novo";
			 
			 //pegar ativos e ultmo passo == 7
			 
			 if($status != '') {
			 $viewbanco = "vw_inscrito";
			 }
			 
			 
			
			$sql = "SELECT * FROM ".$viewbanco." WHERE  ultimo_passo='7'";
